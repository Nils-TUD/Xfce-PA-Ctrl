...
flags = subprocess.check_output(['pkg-config', '--cflags', 'libxfce4panel-1.0 gee-0.8 libpulse libpulse-mainloop-glib']).rstrip()
libs = subprocess.check_output(['pkg-config', '--libs', 'libxfce4panel-1.0 gee-0.8 libpulse libpulse-mainloop-glib']).rstrip()
...
env = Environment(
    ...
    CC = 'clang',
    VC = 'valac',
    VCFLAGS = '--pkg=gtk+-2.0 --pkg=gee-0.8 --pkg=libxfce4panel-1.0 --pkg=libpulse --pkg=libpulse-mainloop-glib --vapidir=contrib',
